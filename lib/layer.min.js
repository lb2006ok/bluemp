! function(window, undefined) {
	...
	Class.pt.set = function(times) {
		...
		switch (config.type) {
			...
			case 1:
				...
				config.title[1] && layerE.find(doms[4]).css({
	...
	Class.pt.autoArea = function(times) {
		...
		var titHeight = config.title[1] ? layerTitle.innerHeight() : 0,
		...
		switch (config.type) {
			...
			case 2:
				layerE.find("iframe").css({
					width: layerE.outerWidth(),
					height: layerE.outerHeight() - (config.title[1] ? layerTitle.innerHeight() : 0)
				});
				break;
			case 3:
	...
	Class.pt.move = function() {
		...
		config.move ? that.layerE.find(config.move).css({
			cursor: "move"
		}) : that.layerE.find(config.move).css({
			cursor: "auto"
		});
